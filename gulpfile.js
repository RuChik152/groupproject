...
gulp.task("myFirstTask", function(param) {
  console.log("Привет, я твой таск");
  param();
});
...
gulp.task("images", function(param) {
  return gulp
    .src("src/images/**/*.{jpg,png,jpeg,svg}")
    .pipe(
      imagemin(
        [
...
gulp.task("browser-sync", function(param) {
  var files = ["dist/index.html", "dist/css/*.css", "dist/js/*.js"];
...
gulp.task(
  "default",
  gulp.series("watch", "browser-sync", "sass", "pug", "scripts", "images")
);
